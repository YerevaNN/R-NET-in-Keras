...
class QuestionAttnGRU(WrappedGRU):
    ...
    def step(self, inputs, states):
        ...
        # uQ_mask = K.any(K.not_equal(uQ, 0), axis=-1, keepdims=True)
        ...
        a_t = K.softmax(s_t)
        c_t = K.batch_dot(uQ, a_t, axes=[1, 1])
        GRU_inputs = K.concatenate([uP_t, c_t])
        g = self.layers['g'].call(GRU_inputs)
        GRU_inputs = g * GRU_inputs
        GRU_states = [vP_tm1,] + list(restStates)
        vP_t, s = super(QuestionAttnGRU, self).step(GRU_inputs, GRU_states)
